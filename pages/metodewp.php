   ...
   <table class="table table-striped ">
      ...
      <tbody>
         ...
         for ($i = 0; $i < $al; $i++) {
            ...
            for ($j = 0; $j < $kr; $j++) {
               if ($atb[$j] == "benefit") {
                  $VekS[$i] *= pow($Valkrt[$i][$j], number_format($bbt[$j], 5));
               } else {
                  $VekS[$i] *= pow($Valkrt[$i][$j], number_format(-$bbt[$j], 5));
               }
               // echo  $Valkrt[$i][$j] . ',';
            ...
            echo "<td>" . number_format($VekS[$i], 4) . "</td>";
            echo "</tr>";
            $nomor++;
         }
...
$datas = [];
for ($i = 0; $i < $al; $i++) {
   $data = ["nama" => $nmal[$i], "nilai" => sprintf("%.4f", $VekS[$i])];
   $datas[] = $data;
}
...
for ($i = 0; $i < $al; $i++) {
   for ($j = 0; $j < count($datas); $j++) {
      if (number_format($VSort[$i], 4) == $datas[$j]["nilai"]) {
...
for ($w = 0; $w < count($datasUpdated); $w++) {
   ...
   $nilaiV = number_format($datasUpdated[$w]["nilai"], 4);
